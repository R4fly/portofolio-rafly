...
interface BuildKeyframesResult {
  window: KeyframeStep[]
  content: KeyframeStep[]
}
...
> = {
  random: null,  // Changed from () => null to just null
...
const buildGrid = ({
  ...
  const orderFn = PATTERNS[pattern] ?? PATTERNS.random
  ...
  for (let row = 0; row < rows; row += 1) {
    for (let column = 0; column < columns; column += 1) {
      ...
      const base = orderFn ? orderFn(x, y) : null
...
const buildKeyframes = ({
  ...
  spin: number
  fade: boolean
}): BuildKeyframesResult => {
  const windowKf: KeyframeStep[] = []
  const contentKf: KeyframeStep[] = []
...
export default function PixelSwap({
  ...
  const renderLayer = (content: ReactNode, index: number): ReactNode => {
