...
export default function MaskedHeading({
  ...
  const settingsRef = useRef<SettingsSnapshot>({} as SettingsSnapshot)
  settingsRef.current = {
    fillScale,
    parallax,
    drift,
  ...
  useEffect(() => {
    ...
    const glyphs = glyphRefs.current.filter(
      (g): g is SVGTextElement => g !== null
    )
    if (!glyphs.length) return

    const riseDistance = (): number =>
